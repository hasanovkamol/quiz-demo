    ...
    <div class="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4"
      ...
      <div class="glass-card rounded-3xl max-w-md w-full p-6 sm:p-8 border border-indigo-500/30 shadow-2xl relative text-center"
        ...
        <div class="flex justify-center mb-4 min-h-[44px]">
          <div #googleBtnContainer></div>
        </div>

        <!-- Custom Google Button Fallback Trigger -->
        <button 
  ...
  loginWithGoogle(): void {
    ...
    this.authService.triggerGoogleSignIn(
      ...
      () => {
        this.isGoogleLoading.set(false);
        this.errorMessage.set(
          'Google Cloud Console da Authorized JavaScript origins sozlanmagan. Iltimos Cloud Console ga http://0.0.0.0:8081 va http://localhost:8081 ni qo\'shing.'
        );
      }
    );
  }
