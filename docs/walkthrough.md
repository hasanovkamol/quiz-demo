...
22. **Graphify AST Knowledge Graph & Ultra-Low Token Agent Navigation**: [`docs/graphify-optimization.md`](./graphify-optimization.md) — Entire codebase parsed into a 767-node / 1369-edge queryable knowledge graph using local tree-sitter AST, eliminating brute-force file scanning token overhead for AI coding assistants.
